...
export default function Upgrade() {
 ...
 <header className={s.header}><nav className={s.nav} aria-label="Navegação principal"><a className={s.brand} href="#topo" aria-label="Beck Performance — início"><span className={s.brandMark}><BrandMark /></span><div>BECK<small>PERFORMANCE</small></div></a><div className={`${s.navLinks} ${menu ? s.menuOpen : ''}`}><a href="#exemplos" onClick={() => setMenu(false)}>Exemplos</a><a href="#como-funciona" onClick={() => setMenu(false)}>Como funciona</a><a href="#perguntas" onClick={() => setMenu(false)}>Perguntas</a></div><a className={s.primary} href="#configurador" onClick={start}>Configurar meu site <span aria-hidden="true">↗</span></a><button className={s.menuButton} aria-label="Abrir navegação" aria-expanded={menu} onClick={() => setMenu(!menu)}>☰</button></nav></header>
 <main><section className={`${s.wrap} ${s.hero}`}><div className={s.heroCopy}><span className={s.eyebrow}><span /> SEU NEGÓCIO, BEM APRESENTADO</span><h1>Seu site começa<br />a tomar <em>forma.</em></h1><p>Escolha, visualize e acompanhe o investimento.<br /><strong>Eu crio e publico o seu site.</strong></p><div className={s.heroPills}><span>Projetos a partir de <b>R$ 500</b></span><span>Prévia sem cadastro</span></div><div className={s.heroActions}><a className={s.primary} href="#configurador" onClick={start}>{hasProgress ? 'Continuar meu projeto' : 'Montar meu site agora'} ↗</a><a className={s.textLink} href="#exemplos">Ver modelos ↓</a></div></div><div className={s.heroShowcase}><div className={s.showcaseNote}><span>SUA IDEIA EM FORMA DE SITE</span><span>PRÉVIA</span></div><ProjectPreview project={{ ...initialProject(), segment: 'beleza', direction: 'elegante', palette: 'azul' }} compact/><div className={s.showcaseBottom}><span className={s.statusDot}/> Visualize antes de contratar</div></div></section>
 ...
 {p.step === 2 && <><p className={s.hint}>Comece pelo essencial. Os opcionais mostram o valor ao lado.</p><div className={s.recommend}><strong>Para {seg.name.toLowerCase()}</strong><p>{p.objective === 'trabalhos' ? 'Uma galeria mostra seu trabalho.' : p.objective === 'localizacao' ? 'Mostre onde encontrar você.' : 'Apresentação, serviços e contato.'}</p><button className={s.textLink} onClick={() => { update({ sections: recommendations(p) }); setNotice('Estrutura recomendada aplicada. Confira os adicionais e a estimativa.'); }}>Aplicar estrutura recomendada ↗</button></div><div className={s.choiceStack}>{sections.map(a => <label key={a.id} className={s.checkRow}><input type="checkbox" checked={p.sections.includes(a.id)} disabled={a.fixed} onChange={() => toggleSection(a.id)}/><span><strong>{a.name}</strong><small>{a.fixed ? 'Já incluído' : a.feature ? 'Opcional' : 'Incluído'}</small></span><b>{a.feature ? `+ ${brl(pricing.byFeature[a.feature])}` : 'Incluído'}</b></label>)}</div><a className={s.helpLink} href="#perguntas">Dúvidas sobre seções, páginas e integrações? Veja as perguntas frequentes ↗</a></>}
 ...
 <section className={`${s.wrap} ${s.author}`}><div className={s.authorMark} aria-hidden="true"><BrandMark /></div><div><span className={s.eyebrow}>BECK PERFORMANCE</span><h2>Você visualiza.<br />Eu faço acontecer.</h2><p>Estratégia, desenvolvimento e publicação.</p><a className={s.textLink} href="/Matheus-Performance/">Conheça meu trabalho ↗</a></div><div className={s.authorProcess}><span>Escopo claro</span><span>Site responsivo</span><span>Acompanhamento até publicar</span></div></section>
 ...
 <footer className={`${s.wrap} ${s.footer}`}><div className={s.brand}><span className={s.brandMark}><BrandMark /></span><div>BECK<small>PERFORMANCE</small></div></div><div><a href={`https://wa.me/${contact.whatsapp}`} target="_blank" rel="noopener noreferrer">WhatsApp</a><Link href="/privacidade/">Privacidade</Link><Link href="/termos/">Termos</Link></div><small>© {new Date().getFullYear()} Beck Performance</small></footer></div>;
}
