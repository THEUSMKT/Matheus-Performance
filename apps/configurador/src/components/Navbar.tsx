  ...
  return (
    <header className="sticky top-0 z-30 border-b border-line/70 bg-canvas/85 backdrop-blur-md">
      <nav className="wrap flex h-16 items-center gap-3 md:gap-8" aria-label="Principal">
        <a href={`${home}#topo`} className="flex min-h-11 flex-none items-center gap-2.5 whitespace-nowrap font-bold tracking-[-0.03em]">
        ...
        <ul className="ml-auto hidden items-center gap-7 text-[0.9375rem] text-muted md:flex">
          {links.map((link) => (
            <li key={link.href}>
              <a href={`${home}${link.href}`} className="inline-flex min-h-11 items-center transition-colors hover:text-ink">
                {link.label}
              </a>
            </li>
          ))}
        ...
        <ButtonLink href={`${home}#configurador`} size="sm" className="ml-auto whitespace-nowrap md:ml-0">
          Configurar meu site
        </ButtonLink>
      </nav>
    </header>
